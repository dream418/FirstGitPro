...
char* ftplib::LastResponse()
{
    ...
    {
        cout << "LastResponse1111" << endl;
        return mp_ftphandle->response;
    }else{
        cout << "LastResponse22222" << endl;
        return NULL;
    }
}
...
int ftplib::Connect(const char *host)
{
    ...
    if (sControl == -1)
    {
        cout << "sControl = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP);  fail"<< endl;
        perror("socket");
        return 0;
    }
    cout << "sControl = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP);  ok"<< endl;
    if (setsockopt(sControl,SOL_SOCKET,SO_REUSEADDR, SETSOCKOPT_OPTVAL_TYPE &on, sizeof(on)) == -1)
    {
        cout << "setsockopt(sControl,SOL_SOCKET,SO_REUSEADDR, SETSOCKOPT_OPTVAL_TYPE &on, sizeof(on)) == -1"<< endl;
        perror("setsockopt");
        net_close(sControl);
        return 0;
    }
    cout << "setsockopt(sControl,SOL_SOCKET,SO_REUSEADDR, SETSOCKOPT_OPTVAL_TYPE &on, sizeof(on)) == ok"<< endl;
    if (connect(sControl, (struct sockaddr *)&sin, sizeof(sin)) == -1)
    {
        cout << "connect(sControl, (struct sockaddr *)&sin, sizeof(sin)) == -1"<< endl;
        perror("connect");
        net_close(sControl);
        return 0;
    }
    cout << "connect(sControl, (struct sockaddr *)&sin, sizeof(sin)) == ok"<< endl;
 ...
 *
 * return 1 if proper response received, 0 otherwise发送命令成功返回1否则为0
 */
 ...
 *
 * return 1 if logged in, 0 otherwise登录成功为1否则为0
 */
...
int ftplib::FtpAccess(const char *path, accesstype type, transfermode mode, ftphandle *nControl, ftphandle **nData)
{
    ...
    sprintf(buf, "TYPE %c", mode);//设置文件传输方式为二进制
    ...
    {
        if (FtpOpenPasv(nControl, nData, mode, dir, buf) == -1) return 0;//进入被动模式传输数据
    }

    if (nControl->cmode == ftplib::port)
    {
        if (FtpOpenPort(nControl, nData, mode, dir, buf) == -1) return 0;//建立一个数据传输的端口连接
        if (!FtpAcceptConnection(*nData,nControl))
        {
            //从服务器接收连接失败
            cout << "从服务器接收连接失败" << endl;
            FtpClose(*nData);
            *nData = NULL;
            return 0;
        }

        cout << "从服务器接收连接成功" << endl;
    }
...
int ftplib::FtpXfer(const char *localfile, const char *path, ftphandle *nControl, accesstype type, transfermode mode)
{
    ...
    if (localfile != NULL)
    {
        ...
        local = fopen64(localfile, ac);//打开文件写入
        if (local == NULL)
        {
            strncpy(nControl->response, strerror(errno), sizeof(nControl->response));
            return 0;
    ...
    if (localfile != NULL) fclose(local);//如果执行命令失败就关闭文件local
    return 0;
  }
 ...
 *
 * return 1 if successful, 0 otherwise发出LIST命令并写入输出响应
 */
